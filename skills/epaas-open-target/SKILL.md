...
- `openType` 是开放对象逻辑字段名。Java/DataRecord 运行态可以通过开放对象字段名读取开放对象配置；SQL 层不要默认认为业务表存在同名物理列。
...
2. 再选择读取方式。
   - 已加载单条 `IDataRecord` 时，可以通过 `bizData.getValue(openType)` 或 `bizData.getRelativeOne(openType)` 获取开放对象配置记录，再读取其中的 `option`。
...
1. 读取开放对象 `option`。在 Java/DataRecord 场景，已加载业务记录可通过 `bizData.getValue(openType)` / `bizData.getRelativeOne(openType)` 获取开放对象配置；也可以使用平台 API 或已确认的关联数据。
2. `ALL` 直接通过。
3. `NONE` 直接拒绝。
...
- SQL 中禁止写成 `t.<openType>` 或 `t.<openType>::jsonb ->> 'option'`，除非已经用源码和运行库确认该业务表确实存有同名物理列。这里的 SQL 约束不影响 Java/DataRecord 运行态通过开放对象字段名读取关联配置。
...
- 业务过账表字段可能是 `varchar`，而服务层参数常是 `Long`；写 `EXISTS` 时必须核对 `task_id/user_id/data_id` 的字段类型和参数 Java 类型，避免 `character varying = bigint`。不能确认时使用显式 `CAST(... AS text)` 或先把参数转为字符串，并在单测中断言渲染 SQL 与参数类型。
...
写 Java/DataRecord 代码时：

- 开放对象字段名可以作为 DataRecord 字段访问入口，例如 `bizData.getValue("data_board_viewers")` 或 `bizData.getRelativeOne("data_board_viewers")`。
- 返回值代表平台装配的开放对象配置记录，不等价于业务表存在 `data_board_viewers` 物理列。
- 读取 `option` 时从开放对象配置记录取 `OpenTargetTable.OPTION`，并明确处理缺失记录、空 option、`NONE`、`ALL`、`PART`。
- 单条直跳权限校验可以使用 DataRecord 字段访问；列表、报表、分页 SQL 仍应使用 `getUserAuthDataPage(...)`、SQL-template 关联 `p_base_open_target`，或消费同步过账表。

## 常见陷阱

- 在 SQL 中把开放对象字段当成业务表 JSON 字段读取。
- 把“DataRecord 可按开放对象字段名取关联配置”误推导为“SQL 可直接读业务表同名列”。
